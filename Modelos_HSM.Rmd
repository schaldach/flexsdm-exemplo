...
p_load(terra, sf, tidyr, dplyr, remotes, viridis, ggcorrplot, RColorBrewer, leaflet, hnp,
...
######> para os dados de ocorrência da espécie e variáveis ambientais
...
Dividindo os dados em partições de treinamento e teste - ao todo, o processo será replicado 10 vezes, e em cada uma o modelo será ajustado 5 vezes, separando cada uma das 5 partições para a validação, a cada vez.
...
## 4.2 Visão geral dos modelos

Visualizando o retorno de cada modelo
...
## 4.3 Resíduos dos modelos

Apenas o GLM e GAM podem ser avaliados desta forma

```{r}
######> GLM
plot(residuals(mglm$model))
hist(residuals(mglm$model))
hnp::hnp(mglm$model)

######> GAM
plot(residuals(mgam$model))
hist(residuals(mgam$model))
# hnp::hnp(mgam$model) # O GAM não funciona

######> Nenhuma das funções funciona para o Maxent
```
...
## 4.5 Dependência entre variáveis e adequação
...
## 4.6 Importância de cada variável ambiental para adequação

Calculando a pontuação de importância de cada variável ambiental com base em permutações

Esse método pode demorar ~20 minutos
...
O resultado disso será um raster, com os valores de -1 a 1, representando as áreas de ganho de adequabilidade (valores positivos) e perdas de adequabilidade (valores negativos)
