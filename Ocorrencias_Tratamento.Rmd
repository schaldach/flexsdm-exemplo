...
csv <- read.table("data/ocorrencias/Sardinella_brasiliensis_full.csv", header = TRUE, sep = ";", dec = ",", fileEncoding = "ISO-8859-1") 
str(csv)
```

```{r}
######> Número de NAs 
sum(csv == "", na.rm = TRUE) # número de strings vazias 
sum(is.na(csv)) # número de NAs

# csv[csv == ""] <- NA # seria necessário caso houvesse strings vazias, iríamos considerar elas como NAs também
...
csv_filtered$x <- csv_filtered$lon
csv_filtered$y <- csv_filtered$lat 
csv_filtered$source <- "csv"
csv_filtered$year <- csv_filtered$ano
```
...
csv_filtered <- csv_filtered[,c("x", "y", "source", "year")] 
excel_filtered <- excel[,c("x", "y", "source", "year")]

######> Juntando todas as linhas 
occurrences <- rbind(csv_filtered, excel_filtered)
str(occurrences) 
...
```{r}
###### Não tem necessidade, pois o polígono já está dentro desse range de latitude 
